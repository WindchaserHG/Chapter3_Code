...
if [ ! -d "./logs_exp/Exp13" ]; then
    mkdir ./logs_exp/Exp13
fi
...
for model_id_name in dataset5 dataset6 dataset7 dataset8; do
    for temp in 10 200; do
        root_path_name="./dataset/ALFA_${model_id_name}"

        # 构建日志文件路径
        log_file="logs_exp/Exp13/${model_id_name}_temp${temp}.log"
