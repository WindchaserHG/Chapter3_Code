...
# 遍历不同的异常比率和序列长度
    for seq_len in 96 192 384; do
        # 构建日志文件路径
        log_file="logs/AnomalyDetection/${model_name}_${model_id_name}_seqlen_${seq_len}_${anomaly_ratio}_d_model_${d_model}.log"
        ...
        --batch_size $batch_size --learning_rate 0.0005 > "$log_file"
    done
done
