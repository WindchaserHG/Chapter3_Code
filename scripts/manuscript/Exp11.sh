...
if [ ! -d "./logs_exp/Exp11" ]; then
    mkdir ./logs_exp/Exp11
fi
...
for model_id_name in dataset2 dataset3 dataset4; do
    for d_model in 8 16; do
        root_path_name="./dataset/ALFA_${model_id_name}"

        # 构建日志文件路径
        log_file="logs_exp/Exp11/${model_id_name}_d_model${d_model}_ar${anomaly_ratio}_inter${no_inter_atten}_intra${no_intra_atten}_con${no_contrastive}.log"
