...
# 创建 logs 目录
if [ ! -d "./logs_exp_path" ]; then
    mkdir ./logs_exp_path
fi

# 创建 logs/AnomalyDetection 目录
if [ ! -d "./logs_exp_path/Exp1" ]; then
    mkdir ./logs_exp_path/Exp1
fi
...
for model_id_name in dataset6; do
    root_path_name="./dataset/ALFA_${model_id_name}"

    # 构建日志文件路径
    log_file="logs_exp_path/Exp1/${model_id_name}.log"
