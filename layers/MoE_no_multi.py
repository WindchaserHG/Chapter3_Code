...
class MoE(nn.Module):
    def __init__(self, input_size, output_size, num_experts, device, num_nodes=1, d_model=32, d_ff=64, dynamic=False,
                 ...
                 no_inter_atten=0, no_intra_atten=0):
        super(MoE, self).__init__()
        self.num_experts = num_experts
        self.output_size = output_size
        self.input_size = input_size
        self.k = k
        self.no_inter_atten = no_inter_atten
        self.no_intra_atten = no_intra_atten
        ...
        self.encoder = Transformer_Layer(device=device, d_model=d_model, d_ff=d_ff,
                                      dynamic=dynamic, num_nodes=num_nodes, patch_nums=1,
                                      patch_size=input_size, factorized=True, layer_number=layer_number, batch_norm=batch_norm, temp=temp,
                                      no_inter_atten=self.no_inter_atten, no_intra_atten=self.no_intra_atten)
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        out, _, weigth = self.encoder(x)
        output = out + x
        contrastive_loss = weigth
