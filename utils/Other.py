...
class FourierLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        b, t, d = x.shape
        x_freq = fft.rfft(x, dim=1)  # (b, t//2+1, d)

        if t % 2 == 0:
            x_freq = x_freq[:, self.low_freq:-1]  # (b, t//2-1, d)
            f = fft.rfftfreq(t)[self.low_freq:-1]  # (t//2-1)
        else:
            x_freq = x_freq[:, self.low_freq:]  # (b, t//2, d)
            f = fft.rfftfreq(t)[self.low_freq:]  # (t//2)

        x_freq, index_tuple = self.topk_freq(x_freq)  # (b, k, d)
        f = repeat(f, 'f -> b f d', b=x_freq.size(0), d=x_freq.size(2))  # (b, k, d)
        f = f.to(x_freq.device)  # (b, k, d)
        f = rearrange(f[index_tuple], 'b f d -> b f () d').to(x_freq.device)  # (b, k, 1, d)

        return self.extrapolate(x_freq, f, t), None  # (b, k, d)
    ...
    def extrapolate(self, x_freq, f, t):
        ...
        amp = rearrange(x_freq.abs() / t, 'b f d -> b f () d') # (b, k, 1, d)
